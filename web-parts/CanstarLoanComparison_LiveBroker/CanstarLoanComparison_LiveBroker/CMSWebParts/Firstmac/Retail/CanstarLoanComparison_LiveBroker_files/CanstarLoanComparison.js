...
var vm_CanstarLoanComparison = function () {
    ...
    self.filter_Features_AdditionalRepayments = ko.observable(false);
    if (localStorage.LB_AdditionalRepayments != null) {
        self.filter_Features_AdditionalRepayments(localStorage.LB_AdditionalRepayments == "true");
    }

    // Lump sum repayments
    self.filter_Features_LumpSumRepayments = ko.observable(false);
    if (localStorage.LB_LumpSumRepayments != null) {
        self.filter_Features_LumpSumRepayments(localStorage.LB_LumpSumRepayments == "true");
    }

    // Offset account
    self.filter_Features_OffsetAccount = ko.observable(false);
    if (localStorage.LB_OffsetAccount != null) {
        self.filter_Features_OffsetAccount(localStorage.LB_OffsetAccount == "true");
    }

    // Pre approval
    self.filter_Features_PreApproval = ko.observable(false);
    if (localStorage.LB_PreApproval != null) {
        self.filter_Features_PreApproval(localStorage.LB_PreApproval == "true");
    }

    // Redraw
    self.filter_Features_Redraw = ko.observable(false);
    if (localStorage.LB_Redraw != null) {
        self.filter_Features_Redraw(localStorage.LB_Redraw == "true");
    }

    // Split loan options
    self.filter_Features_SplitLoanOptions = ko.observable(false);
    if (localStorage.LB_SplitLoanOptions != null) {
        self.filter_Features_SplitLoanOptions(localStorage.LB_SplitLoanOptions == "true");
    }

    // First home buyer
    self.filter_Features_FirstHomeBuyer = ko.observable(false);
    if (localStorage.LB_FirstHomeBuyer != null) {
        self.filter_Features_FirstHomeBuyer(localStorage.LB_FirstHomeBuyer == "true");
    }

    // Investment
    self.filter_Features_Investment = ko.observable(false);
    if (localStorage.LB_Investment != null) {
        self.filter_Features_Investment(localStorage.LB_Investment == "true");
    }

    // Owner occupier
    self.filter_Features_OwnerOccupier = ko.observable(false);
    if (localStorage.LB_OwnerOccupier != null) {
        self.filter_Features_OwnerOccupier(localStorage.LB_OwnerOccupier == "true");
    }
    ...
    self.RefreshResults = function () {
        ...
        localStorage.setItem("LB_AdditionalRepayments", self.filter_Features_AdditionalRepayments());
        localStorage.setItem("LB_FeatureLumpSumRepayments", self.filter_Features_LumpSumRepayments());
        localStorage.setItem("LB_FeatureOffsetAccount", self.filter_Features_OffsetAccount());
        localStorage.setItem("LB_FeaturePreApproval", self.filter_Features_PreApproval());
        localStorage.setItem("LB_FeatureRedraw", self.filter_Features_Redraw());
        localStorage.setItem("LB_SplitLoanOptions", self.filter_Features_SplitLoanOptions());
        localStorage.setItem("LB_FirstHomeBuyer", self.filter_Features_FirstHomeBuyer());
        localStorage.setItem("LB_Investment", self.filter_Features_Investment());
        localStorage.setItem("LB_OwnerOccupier", self.filter_Features_OwnerOccupier());

        //console.log("LoanAmount ticked=" + localStorage.LB_LoanAmount);
        //console.log("FeatureInterestOnly ticked=" + localStorage.LB_FeatureInterestOnly);
        //console.log("LumpSumRepayments not ticked=" + localStorage.LB_FeatureLumpSumRepayments);
...
var getFixedProducts = function (dataURL, ui) {
    jQuery.ajax({
        ...
        success: function (data, self) {
            ...
            var lowestFirstmacInterestRate;
            var FirstmacInterstRateArray = [];

            //get the lowest firstmac interest rate
            for (var i = 0; i < data.length; i++) {
                interstRate = parseFloat(data[i].ADVT_INTEREST_ARTE);
                lenderName = data[i].NAME;
                if (lenderName == "Firstmac") {
                    var FirstmacInterstRate = interstRate;
                    FirstmacInterstRateArray.push(FirstmacInterstRate);
                    console.log("Name==" + lenderName + "FirstmacInterstRate==" + FirstmacInterstRate); //Firstmac
                }
            }
            lowestFirstmacInterestRate = Math.min.apply(Math, FirstmacInterstRateArray.map(function (o) { return o; }));
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                // filter is happening!!!!
                if (interstRate < lowestFirstmacInterestRate) { canShowRecord = false; } //4.19
                ...
                //  console.log("interestonly == " + ui.filter_Features_InterestOnly());
                //   console.log("data[i].INT_PAYMENT_TYPE.toLowerCase()==" + data[i].INT_PAYMENT_TYPE.toLowerCase());

                //additional repayments
                if (ui.filter_Features_AdditionalRepayments() && data[i].ADDITIONAL_REPAYMENT == "No") { canShowRecord = false; }

                //Lump sum repayments
                if (ui.filter_Features_LumpSumRepayments() && data[i].REPAYMENT_ALLOWED == "No") { canShowRecord = false; }

                //Offset account
                if (ui.filter_Features_OffsetAccount() && data[i].OFFSET_DEPOSIT_ACCOUNT == "No") { canShowRecord = false; }

                //Pre approval
                if (ui.filter_Features_PreApproval() && data[i].PRE_APPROVAL_AVAILABLE == "No") { canShowRecord = false; }

                //Redraw
                if (ui.filter_Features_Redraw() && data[i].REDRAW_AVAILABLE == "No") { canShowRecord = false; }

                //Split loan options
                if (ui.filter_Features_SplitLoanOptions() && data[i].SPLIT_OPTION == "No") { canShowRecord = false; }


                //First home buyer
                //if (ui.filter_Features_FirstHomeBuyer() && data[i].ONLY_FIRST_HOME != "Yes") { canShowRecord = false; }

                //Investment
                if (ui.filter_Features_Investment() && data[i].LOAN_PURPOSE != "Investment") { canShowRecord = false; }

                //Owner occupier  
                if (ui.filter_Features_OwnerOccupier() && data[i].LOAN_PURPOSE == "Investment") { canShowRecord = false; }
    ...
    jQuery.ajax({
        ...
        success: function (data, self) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (data[i].ONLY_FIRST_HOME == "Yes") {
                //    console.log("ui.showLenders=="+ui.showLenders +   "only first home==" + data[i].NAME.toLowerCase());
                }
            ...
            //console.log("variable feedResults.lenght=" + ui.feedResults.length);
            jQuery(".productData input[type='checkbox']").click(function () {
                var imagecheck = jQuery(this).parent().find(".imgCheckbox");
                if (!jQuery(this).prop("checked")) {
                    imagecheck.removeClass('tick');
